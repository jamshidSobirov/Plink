...
import com.BibaSpirt.bubble.Bubble;
...
public class GameEngine {
    ...
    public GameEngine(Activity activity, GameView gameView, Observers observers) {
        mActivity = activity;
        mGameView = gameView;
        ...
        mPixelFactor = mScreenWidth / 1600f;
    }
    ...
    public void onGameEvent(GameEvent gameEvent) {
        ...
        for (int i = 0; i < numObjects; i++) {
            try {
                if (((Bubble) mGameObjects.get(i)).mBubbleColor != BubbleColor.BLANK) {
                    cntr++;
                }
            } catch (Exception e) {
                if (mGameObjects.get(i) instanceof Player) {
                    observers.shotBubbleColor(((Player) mGameObjects.get(i)).mBubbleColor);
                }
                e.printStackTrace();
            }
